...
3.  **Methodology** –
    ...
    *In our study, we acknowledge that these are only a few drivers of the SPY. However, for this exploration, our focus is on how much these factors move the market when they are first released*
    ...
    \$\$\$\$\\log\\left(\\frac{P(\\text{high_vol}\_t = 1)}{1 - P(\\text{high_vol}\_t = 1)}\\right)= \\beta_0 + \\beta_1\\,(\\text{CPI_diff})\_t+ \\beta_2\\,(\\text{CPI_day})\_t+ \\beta_3\\,(\\text{Unemp_diff})\_t+ \\beta_4\\,(\\text{Unemp_day})\_t+ \\beta_5\\,(\\text{Fed_diff})\_t+ \\beta_6\\,(\\text{Fed_day})\_t+ \\beta_7\\,(\\text{AllThree})\_t
    $$
