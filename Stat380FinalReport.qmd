...
4.  **Data Analysis Results** – Present and interpret your main findings. Include relevant figures and tables that support your discussion.
    ...
    **Regression Analysis**
    ...
    # 0 = low volatility (at or below median)
    ...
    #Logistic Regression
    log_model <- glm(
      spy_vol ~ cpi_diff + cpi_day + unemp_diff + unemp_day + fed_diff + fed_day + all_three,
      data = train_data,
      family = binomial
    )
    ...
    # Accuracy
    accuracy <- mean(test_pred == test_data$pos_vol)

    # Confusion matrix
    conf_mat <- table(
      Predicted = test_pred,
      Actual = test_data$spy_vol
    )

    # AUC
    roc_obj <- roc(test_data$spy_vol, test_probs)
    auc_val <- auc(roc_obj)

    #Print Results
    cat("Logistic Regression (Positive vs Negative Volatility Change)\n")
    cat("--------------------------------------------------------------\n")
