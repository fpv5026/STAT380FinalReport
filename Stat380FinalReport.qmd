...
3.  **Methodology** –
    ...
    **Regression Analysis:**
    ...
    a\) Our goal is to create a linear regression model where the response variable is SPY volatility and the independent variables are the CPI, Federal Fund Rate, the Unemployment monthly difference (Actual - Predicted). We also have the dummy variables of whether or not the indicator came out that month.
    ...
    *In our study, we acknowledge that these are only a few drivers of the SPY. However, for this exploration, our focus is on how much these factors move the market when they are first released*
    ...
    b\) Our goal is to create a multilogistical regression model where the response variable is high or low SPY volaitlity. We need to look at our market volatility distribution to see if there is a particular threshold of market volatility that we consider high versus low.
    ...
    Our threshold is set to divide the spy_vol by the median. The media of spy_vol is 0.005756937. Therefore, we will consider high vol \> 0.005756937 \> low vol. There are 1030 low vol and 1029 high vol labels. We create a column called high_vol for the labels of high volatility or low volatility.
    ...
    *We acknowledge that high volatility may have a different standard threshold but we are creating this model based on the median market volatility that we have observed in this dataset*
    ...
    The independent variables are still the CPI, Federal Fund Rate, and the Unemployment monthly difference (Actual - Predicted). We also have the days of the release. Our model is predicting whether or not these variables can predict whether the market has high or low volatility (above or below 0.005756937)
...
4.  **Data Analysis Results** – Present and interpret your main findings. Include relevant figures and tables that support your discussion.
    ...
    **cpi_diff**: A one-unit increase in CPI surprise is associated with an odds ratio of `exp(0.19176) = 1.21`. This means the odds of high volatility increase by 21%, holding all other variables constant.
    ...
    **unemp_diff**: A one-unit increase in unemployment surprise is associated with\
    ...
    **fed_diff**: A one-unit increase in the Fed surprise (difference between actual and previous) yields\
    an odds ratio of `exp(-1.55012) = 0.21`. The odds of high volatility decrease by 79% for larger rate-decision surprises.
